...
fun main() {
    val sum = File("src/main/kotlin/advent/day02/day2-input.txt").readText()
        ...
        .flatMap { idRange ->
            ...
            (1..numString.length / 2).forEach { chunkLength ->
                if (numString.length % chunkLength == 0) {
                    val chunkRepeated = numString.take(chunkLength).repeat(numString.length / chunkLength)
                    if (chunkRepeated == numString) {
